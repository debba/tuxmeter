...
Tuxmeter can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.tuxmeter/config.json`
- Default: off
- UI: none

## Config File

Create `~/.tuxmeter/config.json`:
...
- Restart Tuxmeter after changing the file.
...
This applies to provider and plugin HTTP requests that go through Tuxmeter's built-in HTTP client.
