...
Use this when Tuxmeter is not working and you need to share debug info.
...
1. In your macOS menu bar, find the Tuxmeter icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If Tuxmeter does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.sunstory.tuxmeter
```
...
1. Attach `tuxmeter.log`.
2. If you also see files like `tuxmeter.log.1`, attach those too.
