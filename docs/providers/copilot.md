...
1. **Tuxmeter Keychain** (`Tuxmeter-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the Tuxmeter keychain for faster access on subsequent probes.
